...
func GenerarRIDE(factura srixml.FacturaXML, logoPath string) ([]byte, error) {
	...
	if logoPath != "" {
		if _, err := os.Stat(logoPath); err == nil {
			colLogo.Add(image.NewFromFile(logoPath, props.Rect{Center: false, Percent: 80, Left: 0}))
		}
	} else {
		colLogo.Add(text.New("SIN LOGO", props.Text{Style: fontstyle.Italic, Color: textGray}))
	}

	m.AddRow(15,
		colLogo,
		col.New(6).Add(
			text.New("FACTURA ELECTRÓNICA", props.Text{Size: 12, Style: fontstyle.Bold, Align: align.Right, Color: brandPrimary, Top: 0}),
			text.New("No. "+fmt.Sprintf("%s-%s-%s", factura.InfoTributaria.Estab, factura.InfoTributaria.PtoEmi, factura.InfoTributaria.Secuencial), props.Text{Size: 10, Align: align.Right, Top: 6}),
		),
	)
	...
	m.AddRow(12,
		col.New(6).Add(
			text.New(factura.InfoTributaria.RazonSocial, props.Text{Size: 12, Style: fontstyle.Bold, Color: textDark, Top: 0}),
			text.New("RUC: "+factura.InfoTributaria.Ruc, props.Text{Size: 8, Color: textGray, Top: 6}),
		),
		col.New(6).Add(
			text.New("AUTORIZACIÓN / CLAVE DE ACCESO:", props.Text{Size: 7, Style: fontstyle.Bold, Align: align.Right, Color: textGray, Top: 0}),
			text.New(factura.InfoTributaria.ClaveAcceso, props.Text{Size: 7, Align: align.Right, Top: 4, Family: "Courier"}),
		),
	)
	...
	m.AddRow(15,
		col.New(6).Add(
			text.New(factura.InfoTributaria.DirMatriz, props.Text{Size: 7, Color: textGray, Top: 0}),
			text.New("Sucursal: "+factura.InfoFactura.DirEstablecimiento, props.Text{Size: 7, Color: textGray, Top: 4}),
		),
		col.New(6).Add(
			text.New("FECHA: "+factura.InfoFactura.FechaEmision, props.Text{Size: 8, Align: align.Right, Top: 0}),
	...
	m.AddRow(15,
		...
		col.New(6).Add(
			code.NewBar(factura.InfoTributaria.ClaveAcceso, props.Barcode{
				...
				Proportion: props.Proportion{Width: 20, Height: 8},
				Center:     true, // Centrado dentro de su columna (mitad derecha)
			}),
		),
	)
	...
	// =========================================================================
	// 2. DATOS CLIENTE (Fondo Verde Suave)
	// =========================================================================

	m.AddRow(8,
		col.New(12).WithStyle(&props.Cell{BackgroundColor: brandLight}).Add(
			text.New("DATOS DEL CLIENTE", props.Text{Size: 8, Style: fontstyle.Bold, Color: brandPrimary, Top: 2, Left: 2}),
		),
	)

	m.AddRow(18,
		col.New(8).WithStyle(&props.Cell{BackgroundColor: brandLight}).Add(
			text.New("Razón Social: "+factura.InfoFactura.RazonSocialComprador, props.Text{Size: 9, Style: fontstyle.Bold, Left: 2, Top: 2}),
			text.New("Identificación: "+factura.InfoFactura.IdentificacionComprador, props.Text{Size: 9, Left: 2, Top: 8}),
		),
		col.New(4).WithStyle(&props.Cell{BackgroundColor: brandLight}).Add(
			text.New("Dirección: "+factura.InfoFactura.DireccionComprador, props.Text{Size: 8, Align: align.Right, Right: 2, Top: 2}),
			text.New("Teléfono: -", props.Text{Size: 8, Align: align.Right, Right: 2, Top: 8}),
		),
	)
	...
	// =========================================================================
	// 3. TABLA DETALLES (Encabezado Verde Esmeralda)
	// =========================================================================

	m.AddRow(9,
		text.NewCol(2, "CÓDIGO", props.Text{Style: fontstyle.Bold, Size: 8, Align: align.Left, Color: textWhite, Top: 1.5}),
		...
		text.NewCol(2, "P. UNIT", props.Text{Style: fontstyle.Bold, Size: 8, Align: align.Right, Color: textWhite, Top: 1.5}),
		text.NewCol(2, "TOTAL", props.Text{Style: fontstyle.Bold, Size: 8, Align: align.Right, Color: textWhite, Top: 1.5}),
	).WithStyle(&props.Cell{BackgroundColor: brandPrimary})

	for _, item := range factura.Detalles {
		m.AddRow(8,
			text.NewCol(2, item.CodigoPrincipal, props.Text{Size: 8, Align: align.Left, Top: 2, Color: textGray}),
			...
			text.NewCol(2, fmtMoney(item.PrecioUnitario), props.Text{Size: 8, Align: align.Right, Top: 2}),
			text.NewCol(2, fmtMoney(item.PrecioTotalSinImpuesto), props.Text{Size: 8, Align: align.Right, Top: 2, Style: fontstyle.Bold}),
		)
		m.AddRow(1, col.New(12).Add(line.New(props.Line{Color: &props.Color{Red: 230, Green: 230, Blue: 230}})))
	}
	...
	for i := 0; i < totalRowsNeeded+2; i++ {
		...
		if i < len(factura.InfoAdicional) {
			...
			colIzq.Add(text.New(fmt.Sprintf("%s: %s", info.Nombre, info.Value), props.Text{Size: 7, Color: textGray, Top: 0}))
